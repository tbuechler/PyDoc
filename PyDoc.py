...
def extract_inline_code_section(line_inline_comment_starts, lines):
    ...
    section = Section()
    section.addCommentBlock('\n'.join(lines[line_inline_comment_starts:line_inline_comment_ends+1]), True)
...
def create_index_html_file(root_src: str, root_doc: str, parent_link: list=[("Home", "index.html")]):
    ...
    for _file in files:
        table += (r"""- <a href="{}.html">{}</a>""".format(_file.replace('.py', ''), _file.replace('.py', '').replace('_', ' ').title()) + "\n")
